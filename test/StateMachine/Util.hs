...
-- findByRef' :: (Foldable t, Eq a1) => a1 -> t (a2, a1) -> Maybe a2
-- findByRef' ref = foldl' (\acc (u, ref') -> if ref == ref' then Just u else acc) Nothing

updateByRef :: (Foldable t, Eq a) => a -> b -> t (a, b) -> [(a, b)]
updateByRef ref u = foldl' (\acc orig@(ref', _) -> (if ref == ref' then (ref, u) else orig) : acc) []
