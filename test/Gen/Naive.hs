...
-- deriving newtype instance ToADTArbitrary a => ToADTArbitrary (Out a)

-- deriving newtype instance ToADTArbitrary a => ToADTArbitrary (In a)

-- | @since 0.2.0.0
-- >>> sample' $ arbitrary @(UserR "auth")
...
instance ToADTArbitrary (UserR "auth")

-- | @since 0.2.0.0
-- >>> sample' $ arbitrary @(UserR "id")
...
deriving newtype instance Arbitrary (UserR "id")

instance Arbitrary (ArticleR "all") where
  arbitrary = genericArbitrary
  shrink = genericShrink

instance ToADTArbitrary (ArticleR "all")

instance Arbitrary (UserR "profile") where
  arbitrary = genericArbitrary
  shrink = genericShrink

instance ToADTArbitrary (UserR "profile")

instance Arbitrary (ArticleR "withAuthorProfile") where
  arbitrary = genericArbitrary
  shrink = genericShrink

instance ToADTArbitrary (ArticleR "withAuthorProfile")

deriving newtype instance Arbitrary (CommentR "id")

instance Arbitrary (CommentR "withAuthorProfile") where
  arbitrary = genericArbitrary
  shrink = genericShrink

instance ToADTArbitrary (CommentR "withAuthorProfile")

deriving newtype instance Arbitrary (UserR "token")

instance Arbitrary (UserR "authWithToken") where
  arbitrary = genericArbitrary
  shrink = genericShrink

instance ToADTArbitrary (UserR "authWithToken")
