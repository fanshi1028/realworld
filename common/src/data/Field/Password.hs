...
--
-- @since 0.4.0.0
module Data.Field.Password (Password (Password), PasswordHash (PasswordHash), Salt (Salt), hashPassword, checkPassword, mkPassword, newSalt) where

import Data.Aeson (FromJSON (parseJSON))
import Data.Aeson.Types (withText)
import Data.Password.Argon2 (Argon2, PasswordCheck, defaultParams, hashPasswordWithSalt)
import qualified Data.Password.Argon2 as Argon2 (Password, PasswordHash, Salt (Salt), checkPassword, mkPassword)
...
mkPassword = Password . Argon2.mkPassword
...
instance FromJSON (WithValidation Password) where
  parseJSON =
    withText "password" $ \(Argon2.mkPassword -> pw) -> pure $
